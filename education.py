...
def scrape_education_stats(zip_codes=[
    ...
    "48225", "48226", "48227", "48228", "48229", "48230", "49236", "48234", "48235", "48236",
    ...
    # Get already inserted ZIP codes
    existing_zips = set(row[0] for row in cursor.execute("SELECT zip_code FROM education"))
    failed_zips = []
    added_count = 0
    ...
    for zip_code in zip_codes:
        if zip_code in existing_zips:
            continue
        if added_count >= 25:
            print("Limit of 25 ZIPs reached for this run.")
            break
    ...
    print(f"Saved {added_count} new ZIPs")
    if failed_zips:
        with open("missing_education_data.txt", "w") as f:
            f.write("The following ZIP codes could not be processed:\n")
            f.write("\n".join(failed_zips))
        print(f"Logged {len(failed_zips)} missing ZIPs to missing_education_data.txt")
